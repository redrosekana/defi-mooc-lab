...
describe("LiquidationOperator2", function () {

  // 2000
  it("Test flash loan with 2000 USDT so that liquidate", async function () {
    console.log('------------------------------------------')
    ...
    const gasPrice = 0;
    const debt = "2000";
    const usdtValue = ethers.utils.parseUnits(debt, 6);
    ...
    const LiquidationOperator2 = await ethers.getContractFactory("LiquidationOperator2");
    const liquidationOperator = await LiquidationOperator2.deploy(usdtValue,overrides = {gasPrice: gasPrice});
    ...
    console.log(
      `Profit which is from using 2000 USDT for the liquidation is`,
  ...
  // 5000
  it("Test flash loan with 5000 USDT so that liquidate", async function () {
    console.log('------------------------------------------')
    ...
    const profit = afterLiquidationBalance.sub(beforeLiquidationBalance);
    console.log(
      `Profit which is from using 5000 USDT for the liquidation is`,
      utils.formatEther(profit),
      "ETH"
    );
  ...
  // 10000
  it("Test flash loan with 10000 USDT so that liquidate", async function () {
    console.log('------------------------------------------')
    ...
    console.log("transaction reverted not liquidate for 10000 USDT")
  });
});
